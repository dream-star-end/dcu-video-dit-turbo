...
_H3_15S_LOCAL_M = frozenset((11819, 12055, 12280, 18873, 18874, 36711, 36712, 54549, 54550))  # T2V, first-frame I2V, first+last FLF
_VEC4_EPILOGUE_M = _H3_15S_LOCAL_M
...
# Exact gfx936 activation quantizer.  The DSO fuses row absmax, FP32 scale,
# BF16 division/round-even and INT8 store without changing any result bits.
# Keep it opt-in and hard-fail at startup when explicitly requested so an ABI
# problem can never silently turn a benchmark into the eager baseline.
_exact_quantizer_enabled = os.environ.get("H3_INT8_EXACT_QUANTIZER", "0") == "1"
_exact_quantizer_so = os.environ.get(
    "H3_INT8_EXACT_QUANTIZER_SO",
    "kernels/exact-rowwise-int8/build/"
    "h3_exact_rowwise_int8.so",
)
_exact_quantizer = None
_exact_quantizer_hits = 0
_exact_quantizer_fallbacks = Counter()
_EXACT_QUANTIZER_M = _H3_15S_LOCAL_M
...
def _load_exact_quantizer():
    ...
    if not hasattr(module, "quantize_int8_rowwise"):
        raise RuntimeError(
            "H3 exact INT8 quantizer DSO lacks quantize_int8_rowwise: "
            f"{_exact_quantizer_so}"
        )
    _exact_quantizer = module
    return module


def _exact_quantizer_gate(
    x: torch.Tensor, stochastic_rounding: int
) -> tuple[bool, str]:
    ...
    if x.dim() != 2 or x.shape[0] not in _EXACT_QUANTIZER_M:
        return False, "M"
...
def _quantize_activation(x: torch.Tensor, stochastic_rounding: int = 0):
    ...
    if _exact_quantizer_fallbacks[reason] == 1:
        logging.warning(
            "H3 exact INT8 quantizer fallback reason=%s shape=%s dtype=%s "
            "contiguous=%s",
            reason,
            tuple(x.shape),
            x.dtype,
...
# torch._int_mm accepts the three measured T2V/I2V/FLF local row counts
# directly.  Every listed shape is validated element-for-element against the
# padded implementation before this launcher is deployed.
_H3_15S_INT8_SHAPES = {
    (m, k, n)
    for m in _H3_15S_LOCAL_M
    for k, n in (
        (5376, 21504),
        (7168, 5376),
        (5376, 28672),
...
def _vec4_epilogue_gate(
    ...
    if accumulator.dim() != 2 or accumulator.shape[0] not in _VEC4_EPILOGUE_M:
        return False, "M"
...
if _vec4_epilogue_enabled:
    ...
    _load_vec4_epilogue()
    logging.warning(
        "H3 exact vec4 epilogue loaded: %s contract=gfx936-M11819,12055,12280-"
...
if os.environ.get("H3_STREAM_ENCODE", "0") == "1":
    _h3_vae.MiniMaxH3VideoVAE.decode_temporal._h3_sp_contract = (
        "v7-21chunks-cut11"
    )
    runpy.run_path(
        os.path.join(os.path.dirname(__file__), "h3_stream_encode_candidate.py"),
        run_name="_h3_stream_encode_candidate",
    )

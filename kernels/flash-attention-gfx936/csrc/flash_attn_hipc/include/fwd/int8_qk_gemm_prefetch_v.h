#pragma once
#include "int8_qk_gemm_prefetch_v_headdim128.h"
...
__forceinline__ __device__ void  int8_qk_gemm_prefetch_v(
        vec4_uint gQ,
        vec4_uint gK,
        vec4_uint gV,
        Element_k* q_lds,
    ...
    union_vec4_f16x2<int8_t> k_reg[STAGES*((WARP_N*kBlockK)/(32*64))*2];
    ...
    for(int k_loop = K_LOOP_START; k_loop < QK_LOOP_COUNT; ++k_loop) {
        ...
        int k_lds_stage_offset = stage_id * (WARP_N / 32) * (kBlockK / 64) * (32 * 17);
                ...
                for(int n_idx = 0; n_idx < (WARP_N / 32); ++n_idx) {
                    ...
                    for(int head_dim_idx = 0; head_dim_idx < (kBlockK / 64); ++head_dim_idx) {
                        ...
                        for(int j = 0; j < 2; ++j) {
                            ...
                            inline_ds_read2_b32_no_wait_bytes(lds_offset, k_reg[stage_id * (WARP_N * kBlockK) / (32 * 64) * 2 + (head_dim_idx * (WARP_N / 32) + n_idx) * 2 + i].u64[j], 2);
                        }
                    }
                }
            }
                    ...
                    for(int min_tile_k = 0; min_tile_k < 2; ++min_tile_k) {
                        ...
                        for(int n_idx = 0; n_idx < (WARP_N / 32); ++n_idx) {
                            ...
                            for(int min_tile_m = 0; min_tile_m < 2; ++min_tile_m) {
                                ...
                                int k_tile_id  = stage_id * (WARP_N * kBlockK) / (32 * 64) * 2 + (head_dim_idx * (WARP_N / 32) + n_idx) * 2 + min_tile_n;
    ...
    if constexpr (STAGES == 2) {
        ...
        if constexpr (true) {
            // vec2_Element<Element> *k_lds_v2fp16 =  (vec2_Element<Element> *)(k_lds);
            vec4_int8 *k_lds_v4int8 =  (vec4_int8 *)(k_lds);
            int k_lds_stage_offset = last_stage_id * (WARP_N / 32) * (kBlockK / 64) * (32 * 17);
            for(int head_dim_idx = 0; head_dim_idx < (kBlockK / 64); ++head_dim_idx) {
                for(int n_idx = 0; n_idx < (WARP_N / 32); ++n_idx) {
                    for(int i = 0; i < 2; ++i) {
                        for(int j = 0; j < 2; ++j) {
            ...
            for(int head_dim_idx = 0; head_dim_idx < (kBlockK / 64); ++head_dim_idx) {
                ...
                for(int n_idx = 0; n_idx < (WARP_N / 32); ++n_idx) {
                    for(int i = 0; i < 2; ++i) {
                        for(int j = 0; j < 2; ++j) {
                            int lds_offset = precompute_k_lds_offset[i * 2 + j];
                            inline_ds_read2_b32_no_wait_bytes(lds_offset, k_reg[last_stage_id * (WARP_N * kBlockK) / (32 * 64) * 2 + (head_dim_idx * (WARP_N / 32) + n_idx) * 2 + i].u64[j], 2);
                        }
                    }
                }
            }
                ...
                for(int head_dim_idx = 0; head_dim_idx< (kBlockK / 64); ++head_dim_idx) {
                    ...
                    for(int min_tile_k = 0; min_tile_k < 2; ++min_tile_k) {
                        ...
                        for(int n_idx = 0; n_idx < (WARP_N / 32); ++n_idx) {
                            #pragma unroll
                            for(int min_tile_m = 0; min_tile_m < 2; ++min_tile_m) {
                                int q_tile_id  = (QK_LOOP_COUNT - 1) * (WARP_M * kBlockK) / (32 * 64) * 2 + (head_dim_idx * (WARP_M / 32) + m_idx) * 2 + min_tile_m;
                                int k_tile_id  = last_stage_id * (WARP_N * kBlockK) / (32 * 64) * 2 + (head_dim_idx * (WARP_N / 32) + n_idx) * 2 + min_tile_n;
                                int s_tile_id  = n_idx * (WARP_M / 32) + m_idx;
                                int8_mmac();
                            }
                        }

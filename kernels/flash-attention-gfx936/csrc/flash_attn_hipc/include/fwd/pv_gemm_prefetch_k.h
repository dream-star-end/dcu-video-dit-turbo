...
__forceinline__ __device__ void prefix_prefill_hdim512_16x64_prefetch_v_to_lds(
        vec4_uint gV,
        Element* v_lds,
        const int warp_id,
        const int lane_id,
    ...
    const int col = d_loop * kBlockK + (lane_id & 3) * 8;
    const int lds_offset = row_raw * kBlockK + (lane_id & 3) * 8;
    safe_inline_buffer_load_dwordx4_lds<Element, 1>(
        v_lds, gV, lds_offset, 0, row * v_row_stride + col);
}

template<int kHeadDimV, int kBlockN, int kBlockK, int WARP_M, typename Element, typename ElementAccum, bool Is_even_MN>
__forceinline__ __device__ void prefix_prefill_hdim512_16x64_pv(
        vec4_uint gV,
        Element* v_lds,
    ...
    #pragma unroll
    for (int d_loop = 0; d_loop < kHeadDimV / kBlockK; ++d_loop) {
        prefix_prefill_hdim512_16x64_prefetch_v_to_lds<kBlockN, kBlockK, WARP_M, Element, Is_even_MN>(
            gV, v_lds, warp_id, lane_id, v_row_stride, max_seq_k_offset, d_loop);
        ...
        for (int n_loop = 0; n_loop < kBlockN / 16; ++n_loop) {
            ...
            #pragma unroll
            for (int i = 0; i < 4; ++i) {
                const int v_load_offset = n_loop * 16 * kBlockK + i * 4 * kBlockK + (lane_id >> 4) * kBlockK + (lane_id & 15) * 2;
                v_regs.f16x2[i] = *reinterpret_cast<vec2_Element<Element>*>(v_lds + v_load_offset);
            }
            union_vec4_f16x2<Element> v_composed;
            v_composed.f16x4[0] = make_vec4_f16<Element>(
                v_regs.f16[0], v_regs.f16[2], v_regs.f16[4], v_regs.f16[6]);
            v_composed.f16x4[1] = make_vec4_f16<Element>(
                v_regs.f16[1], v_regs.f16[3], v_regs.f16[5], v_regs.f16[7]);

            acc_o[d_loop * 2 + 0].f32 = mmac<Element, ElementAccum>(
                p_reg[n_loop].f16x4, v_composed.f16x4[0], acc_o[d_loop * 2 + 0].f32);
            acc_o[d_loop * 2 + 1].f32 = mmac<Element, ElementAccum>(
                p_reg[n_loop].f16x4, v_composed.f16x4[1], acc_o[d_loop * 2 + 1].f32);
        }

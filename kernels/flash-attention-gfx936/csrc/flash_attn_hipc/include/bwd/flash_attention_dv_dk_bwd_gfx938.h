...
__forceinline__ __device__ void compute_dk_dv_1colblock_gfx938(Params &params, int bidb, int bidh, int n_block
    ...
    const int m_block_min = (!Is_causal && !Is_local) ? 0 : std::max(0, (n_block * kBlockN_ - seqlen_k_minus_q - params.window_size_right) / kBlockM_);
    const int m_block_max = !Is_local ? ceil_div(binfo.actual_seqlen_q, kBlockM_) : std::min(ceil_div(binfo.actual_seqlen_q, kBlockM_), ceil_div((n_block + 1) * kBlockN_ - seqlen_k_minus_q + params.window_size_left, kBlockM_));
    ...
    for (int m_block = m_block_max - 1; m_block >= m_block_min; --m_block) {
        ...
        */
        ...
        apply_mask_bwd_gfx938<Is_even_MN, Is_local ? 3 : (Is_causal ? 2 : 0)>(s_reg, binfo.actual_seqlen_k - n_block * kBlockN_ - warp_id * 32, binfo.actual_seqlen_q - m_block * kBlockM_, (n_block * kBlockN_ + warp_id * 32 - seqlen_k_minus_q) - m_block * kBlockM_, params.window_size_right, params.window_size_left);
        ...
        if constexpr (Is_dropout) {
            ...
            const int block_col_start = n_block * (kBlockN_ / 32) + (warp_id & (WARP_N_ / 32 - 1));
            const int block_row_start = m_block * (kBlockM_ / 32) + (warp_id / (WARP_N_ / 32));
            apply_dropout<true, union_vec4_fp32, kBlockM_, WARP_N_>(
                s_reg,
        ...
        if constexpr (Is_dropout || !Is_even_K || std::is_same_v<Element, bhalf_t>) {
            __builtin_amdgcn_sched_barrier(0);
            __builtin_amdgcn_s_waitcnt(0);
            __syncthreads();
            __builtin_amdgcn_sched_barrier(0);
                ...
                for(int k_tile_idx=0; k_tile_idx<(kBlockK_/32); k_tile_idx++) {
                    ...
                    for(int min_tile_n=0; min_tile_n<2; min_tile_n++) {
                        ...
                        for(int vec_index=0;  vec_index<4; vec_index++) {
                            ...
                            const int seq_idx = n_block * kBlockN_ + warp_id * WARP_N_ + warp_n_idx * 32 + dv_lane_seq_idx + min_tile_n * 16 + vec_index * 4;
                            if constexpr (Is_even_K) {
                                ...
                                if (seq_idx < binfo.actual_seqlen_k) {
                                    if (head_idx < params.d_value) {
                                        dv_ptr[global_addr] = v_data[0];
                                    }
                                    if (head_idx + 1 < params.d_value) {
                                        dv_ptr[global_addr + 1] = v_data[1];
                                    }
                                }
                            }
                        }
                ...
                for(int k_tile_idx=0; k_tile_idx<(kBlockK_/32); k_tile_idx++) {
                    ...
                    for(int min_tile_n=0; min_tile_n<2; min_tile_n++) {
                        ...
                        for(int vec_index=0;  vec_index<4; vec_index++) {
                            ...
                            const int seq_idx = n_block * kBlockN_ + warp_id * WARP_N_ + warp_n_idx * 32 + dk_lane_seq_idx + min_tile_n * 16 + vec_index * 4;
                            if constexpr (Is_even_K) {
                                if (Is_even_MN || seq_idx < binfo.actual_seqlen_k) {
                                ...
                                if (seq_idx < binfo.actual_seqlen_k) {
                                    if (head_idx < params.d) {
                                        dk_ptr[global_addr] = v_data[0];
                                    }
                                    if (head_idx + 1 < params.d) {
                                        dk_ptr[global_addr + 1] = v_data[1];
                                    }
                                }
                            }
                        }

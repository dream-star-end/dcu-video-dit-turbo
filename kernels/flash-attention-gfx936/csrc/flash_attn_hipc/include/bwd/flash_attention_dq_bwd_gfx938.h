...
__forceinline__ __device__ void compute_dq_1colblock_gfx938(Params &params, int bidb, int bidh, int m_block
    ...
    const int n_block_min = !Is_local ? 0 : std::max(0, (m_block * kBlockM_ + seqlen_k_minus_q - params.window_size_left) / kBlockN_);
    const int n_block_max = (!Is_causal && !Is_local) ? ceil_div(binfo.actual_seqlen_k, kBlockN_) : std::min(ceil_div(binfo.actual_seqlen_k, kBlockN_), flash::ceil_div((m_block + 1) * kBlockM_ + seqlen_k_minus_q + params.window_size_right, kBlockN_));
    ...
    for (int n_block = n_block_max - 1; n_block >= n_block_min ;  --n_block) {
        ...
        *(uint64_t*)&gV -= ((kBlockN_ * seqlen_v_stride) * sizeof(Element));
        ...
        apply_mask_bwd_gfx938<Is_even_MN, Is_local ? 3 : (Is_causal ? 1 : 0)>(s_reg, binfo.actual_seqlen_q - m_block * kBlockM_ - warp_id * 32, binfo.actual_seqlen_k - n_block * kBlockN_, (m_block * kBlockM_ + warp_id * 32 + seqlen_k_minus_q) - (n_block * kBlockN_), params.window_size_left, params.window_size_right);
                    ...
                    for(int min_tile_m=0; min_tile_m<2; min_tile_m++) {
                        ...
                        for(int vec_index=0;  vec_index<4; vec_index++) {
                            ...
                            for(int min_tile_k=0; min_tile_k<2; min_tile_k++) {
                                ...
                                const int seq_idx = (m_block * kBlockM_) + (warp_id * WARP_M_ + warp_m_idx * 32 + dq_lane_seq_idx) + min_tile_m * 16 + vec_index * 4;

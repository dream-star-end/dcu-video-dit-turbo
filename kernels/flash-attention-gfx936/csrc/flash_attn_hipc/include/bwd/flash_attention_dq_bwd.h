...
__forceinline__ __device__ void compute_dq_1colblock(Params &params, int bidb, int bidh, int m_block
    ...
    const int n_block_count = ceil_div(binfo.actual_seqlen_k, kBlockN_);
    const int n_block_min = !Is_local ? 0
        : std::max(0, (m_block * kBlockM_ + seqlen_k_minus_q - params.window_size_left) / kBlockN_);
    const int n_block_max_unclamped = (!Is_causal && !Is_local) ? n_block_count
        : std::min(n_block_count, flash::ceil_div((m_block + 1) * kBlockM_ + seqlen_k_minus_q + params.window_size_right, kBlockN_));
    const int n_block_max = (Is_causal || Is_local) ? std::max(1, n_block_max_unclamped) : n_block_max_unclamped;
        ...
        *(uint64_t*)&gV -= ((kBlockN_ * seqlen_v_stride) * sizeof(Element));
        ...
        if constexpr (!Is_causal && !Is_local) {
            if constexpr (!Is_even_MN) {
                apply_mask<vec4_Accum<ElementAccum>, WARP_M_, kBlockN_>(s_reg, binfo.actual_seqlen_k - n_block * kBlockN_);
            }
        } else {
            if constexpr (Is_local) {
                apply_mask_local<Is_local, vec4_Accum<ElementAccum>, WARP_M_, kBlockN_>(
                    s_reg,
                    n_block * kBlockN_,
                    binfo.actual_seqlen_k,
                    m_block * kBlockM_ + warp_id * WARP_M_,
                    binfo.actual_seqlen_q,
                    params.window_size_left,
                    params.window_size_right);
            } else if constexpr (Is_causal) {
                if (binfo.actual_seqlen_q == binfo.actual_seqlen_k) {
                    apply_mask_bwd<Is_even_MN, 1>(
                        s_reg,
                        binfo.actual_seqlen_q - m_block * kBlockM_ - warp_id * WARP_M_,
                        binfo.actual_seqlen_k - n_block * kBlockN_,
                        (m_block * kBlockM_ + warp_id * WARP_M_) - (n_block * kBlockN_),
                        params.window_size_left,
                        params.window_size_right);
                } else {
                    apply_mask_causal<vec4_Accum<ElementAccum>, WARP_M_, kBlockN_>(
                        s_reg,
                        n_block * kBlockN_,
                        binfo.actual_seqlen_k,
                        m_block * kBlockM_ + warp_id * WARP_M_,
        ...
        if constexpr (Is_dropout) {
            const unsigned long long seed = params.rng_state[0];
            const unsigned long long offset = params.rng_state[1] + ((bidb * params.h + bidh) << 6) + lane_id;
            union_vec2_uint rowcol;
            rowcol.u32.x = m_block * (kBlockM_ / 32) + warp_id;
            rowcol.u32.y = n_block * (kBlockN_ / WARP_N_);
            apply_dropout<true, vec4_Accum<ElementAccum>, WARP_M_, kBlockN_, kBlockN_ / WARP_N_, Is_even_MN>(
                s_reg,
                binfo.actual_seqlen_q,
                binfo.actual_seqlen_k,
                m_block * kBlockM_ + warp_id * WARP_M_,
                    ...
                    for(int min_tile_m=0; min_tile_m<2; min_tile_m++) {
                        ...
                        for(int vec_index=0;  vec_index<4; vec_index++) {
                            ...
                            for(int min_tile_k=0; min_tile_k<2; min_tile_k++) {
                                const int head_idx = dq_block_buffer_store_global_offset + k_tile_idx * 32 + min_tile_k + dq_lane_head_dim_idx * 2;
                                const int seq_idx = (m_block * kBlockM_) + (warp_id * WARP_M_ + warp_m_idx * 32 + dq_lane_seq_idx * 2) + min_tile_m + vec_index * 8;
                                int dq_global_addr = dq_global_addr_offset + (min_tile_m + vec_index*8)*seqlen_dq_stride + min_tile_k  + dq_lane_head_dim_idx*2;
                                if((Is_even_MN || seq_idx < binfo.actual_seqlen_q) && (Is_even_K || head_idx < params.d)) {

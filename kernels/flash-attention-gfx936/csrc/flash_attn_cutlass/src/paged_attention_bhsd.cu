...
__launch_bounds__(NUM_THREADS) __global__ void paged_attention_kernel_bhsd(
  ...
  for (int block_idx = start_block_idx; block_idx < end_block_idx; block_idx ++) {
    ...
    if(block_idx < num_seq_blocks-1){
      #pragma unroll
      for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
        int offset=i*HEAD_SIZE/NUM_ROWS_PER_THREAD+warp_idx*HEAD_SIZE/NUM_ROWS_PER_THREAD/NUM_WARPS+rows*4*vecsize*HEAD_SIZE+rowid;
        half4_vec v_vec;
        scalar_t *p=(scalar_t*)&v_vec;
        if constexpr(is_fp8){
          for(int ii=0;ii<vecsize*4;ii++){
            p[ii]=uint82half<scalar_t,is_e4m3>(v_ptr[offset+ii*HEAD_SIZE]);
          }
        }else{
          for(int ii = 0 ; ii<4*vecsize ; ii++){
      ...
      for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
        ...
        if constexpr(is_fp8){
          for(int ii=0;ii<vecsize*4;ii++){
            p[ii]=token_idx + ii < seq_len ? uint82half<scalar_t,is_e4m3>(v_ptr[offset+ii*HEAD_SIZE]) : 0;
          }
        }else{
          for(int ii = 0 ; ii<4*vecsize ; ii++){
      ...
      Cache_Type_SWITCH(scalar_t,key_cache.dtype(),[&] {
        REUSEKV_SWITCH(reusekv,[&] {
          BOOL_SWITCH(block_size==64,is_block64,[&]{
            ...
            paged_attention_kernel_bhsd<scalar_t,cache_t,false,HEAD_SIZE,BLOCK_SIZE,NUM_THREADS,REUSE_KV_TIMES><<<grid,block,shared_mem_size,stream>>>(

...
from flash_attn.flash_attn_interface import _flash_attn_varlen_forward, _get_block_size_n
...
def attention_ref(
    ...
    # 为了数值稳定性，einsum 累加始终使用 float32
    v_scaled = v * dropout_scaling
    if not upcast and dtype_og in [torch.float16, torch.bfloat16]:
        output = torch.einsum("bhts,bshd->bthd", 
...
def make_cu_seqlens(lengths, device):
    return torch.tensor([0, *torch.cumsum(torch.tensor(lengths), dim=0).tolist()], device=device, dtype=torch.int32)
...
def attention_varlen_ref(
    ...
    for batch_idx in range(batch_size):
        ...
        if k_i.shape[0] == 0:
            outputs.append(torch.zeros(q_i.shape[0], nheads, v.shape[-1], device=q.device, dtype=v.dtype))
            lse_chunks.append(torch.full((nheads, q_i.shape[0]), float("inf"), device=q.device, dtype=torch.float32))
            continue

        scores = torch.einsum("thd,shd->hts", q_i / math.sqrt(d), k_i)
        if causal:
            causal_mask = torch.triu(
                torch.ones(q_i.shape[0], k_i.shape[0], dtype=torch.bool, device=q.device),
                diagonal=k_i.shape[0] - q_i.shape[0] + 1,
            )
        ...
        outputs.append(torch.einsum("hts,shd->thd", attention.to(v_i.dtype), v_i))

        lse_i = scores.logsumexp(dim=-1)
        lse_i[lse_i == float("-inf")] = float("inf")
        lse_chunks.append(lse_i)

    output = torch.cat(outputs, dim=0).to(dtype=dtype_og)
    if not return_lse:
        return output
    lse = torch.cat(lse_chunks, dim=-1).float()
    return output, lse
...
# @pytest.mark.parametrize('seqlen_q,seqlen_k', [(128, 64)])
@pytest.mark.parametrize("dropout_p", [0.0, 0.17])
# @pytest.mark.parametrize("dropout_p", [0.0])
...
def test_flash_attn_output(
    seqlen_q, seqlen_k, d, dropout_p, causal, local, alibi, deterministic, mha_type, dtype, kvpacked, softcap, bhsd
):
    if (
    ...
    if kvpacked:
        ...
        if bhsd:
            kv_bhsd = kv.transpose(3, 1).contiguous()
    else:
        k = torch.randn(
    ...
    if (d <= MAX_HEADDIM_SM8x or dropout_p == 0) or (is_sm80 or is_sm90):
        if kvpacked:
            ...
            (
                dq_ref,
                dkv_ref,
            ) = torch.autograd.grad(out_ref, (q, kv), g if not bhsd else g_bshd)
            dk_ref, dv_ref = dkv_ref.unbind(2)
            (
                dq_pt,
                dkv_pt,
            ) = torch.autograd.grad(out_pt, (q, kv), g if not bhsd else g_bshd)
            ...
            (
                ...
                dk_ref,
                dv_ref,
            ) = torch.autograd.grad(out_ref, (q, k, v), g if not bhsd else g_bshd)
            (
                dq_pt,
                dk_pt,
                dv_pt,
            ) = torch.autograd.grad(out_pt, (q, k, v), g if not bhsd else g_bshd)
        if bhsd:
...
# @pytest.mark.parametrize("softcap", [0.0])
@pytest.mark.parametrize("bhsd", [False])
def test_flash_attn_output_mla(
    seqlen_q, seqlen_k, d, d_v, dropout_p, causal, local, alibi, mha_type, dtype, softcap, bhsd
):
    if (
        max(seqlen_q, seqlen_k) >= 2048
        and torch.cuda.get_device_properties("cuda").total_memory <= 16 * 2**30
    ...
    v = torch.randn(
        batch_size, seqlen_k, nheads_k, d_v, device=device, dtype=dtype, requires_grad=True
    )
    if bhsd:
        k_bhsd = k.transpose(2, 1).contiguous()
        v_bhsd = v.transpose(2, 1).contiguous()
...
@pytest.mark.parametrize("softcap", [0.0, 50.0])
# @pytest.mark.parametrize("softcap", [0.0])
def test_flash_attn_varlen_output_mla(
    ...
    #         assert abs(dropout_fraction - dropout_p) <= (0.01 if not local else 0.04)
    # return
    # torch.nonzero((dk - dk_ref).abs()>0.01)
    # pdb.set_trace()
    if ((d <= MAX_HEADDIM_SM8x or (d > 224 and dropout_p == 0)) or (is_sm80 or is_sm90)) and softcap == 0.0:
        assert (dq - dq_ref).abs().max().item() <= 3 * (dq_pt - dq_ref).abs().max().item()
        assert (dk - dk_ref).abs().max().item() <= 3 * (dk_pt - dk_ref).abs().max().item()
...
@pytest.mark.parametrize("softcap", [0.0, 50.0])
# @pytest.mark.parametrize("softcap", [0.0])
def test_flash_attn_varlen_output_mla_gfx928(
    seqlen_q, seqlen_k, d, d_v, dropout_p, causal, local, alibi, deterministic, mha_type, dtype, softcap
):
    if (
        max(seqlen_q, seqlen_k) >= 2048
...
# @pytest.mark.parametrize("seqlen_q,seqlen_k", [(256, 128)])
def test_flash_attn_varlen_causal_mla(
    seqlen_q, seqlen_k, swap_sq_sk, d, d_v, local, paged_kv_block_size, dtype
):
    if (
        max(seqlen_q, seqlen_k) >= 2048
        and torch.cuda.get_device_properties("cuda").total_memory <= 16 * 2**30
    ...
    if paged_kv_block_size is None:
        ...
        v = torch.randn(
            batch_size, seqlen_k, nheads, d_v, device=device, dtype=dtype, requires_grad=True
        )
        block_table = None
    else:
        k, v, block_table, k_cache_paged, v_cache_paged, num_blocks = _generate_block_kvcache_mla(
            seqlen_k, paged_kv_block_size, batch_size, nheads, d, d_v, device, dtype
        )
...
#     ],
# )
@pytest.mark.parametrize('seqlen_q,seqlen_k', [(8552, 8552)])
...
def test_flash_attn_kvcache(
    ...
    # print("out ", out[0, 0, 0, :10])
    # print("out ref ", out_ref[0, 0, 0, :10])
...
# @pytest.mark.parametrize('seqlen_q,seqlen_k', [(256, 128)])
def test_flash_attn_kvcache_mla(
    seqlen_q,
    seqlen_k,
    d,
    d_v,
...
def _generate_block_kvcache_mla(seqlen_k, paged_kv_block_size, batch_size, nheads_k, d, d_v, device, dtype):
    num_blocks = math.ceil(seqlen_k / paged_kv_block_size) * batch_size * 3
    k_cache_paged = torch.randn(
        num_blocks, paged_kv_block_size, nheads_k, d, device=device, dtype=dtype
    )
...
def _assert_varlen_out_and_lse_close(out, lse, out_ref, lse_ref, out_pt, lse_pt):
    out_max_diff = (out - out_ref).abs().max().item()
    out_pt_max_diff = (out_pt - out_ref).abs().max().item()
    assert out_max_diff <= max(2 * out_pt_max_diff, 1e-4) + 1e-5

    assert torch.equal(torch.isinf(lse), torch.isinf(lse_ref))
    finite_mask = torch.isfinite(lse_ref)
    if finite_mask.any():
        lse_max_diff = (lse[finite_mask] - lse_ref[finite_mask]).abs().max().item()
        lse_pt_max_diff = (lse_pt[finite_mask] - lse_ref[finite_mask]).abs().max().item()
        assert lse_max_diff <= max(2 * lse_pt_max_diff, 1e-4) + 1e-5


@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_flash_attn_varlen_tiny_dim64_ragged_causal_matches_ref(dtype):
    torch.random.manual_seed(0)
    device = "cuda"
    nheads = 8
    d = 64
    ...
    q = torch.randn(sum(seqlens_q), nheads, d, device=device, dtype=dtype)
    k = torch.randn(sum(seqlens_k), nheads, d, device=device, dtype=dtype)
    v = torch.randn(sum(seqlens_k), nheads, d, device=device, dtype=dtype)

    out, sm_lse, _, _ = _flash_attn_varlen_forward(
        q,
        k,
        v,
        cu_seqlens_q,
    ...
    out_ref, lse_ref = attention_varlen_ref(q, k, v, cu_seqlens_q, cu_seqlens_k, causal=True, return_lse=True)
    out_pt, lse_pt = attention_varlen_ref(
        q, k, v, cu_seqlens_q, cu_seqlens_k, causal=True, upcast=False, return_lse=True
    )
    _assert_varlen_out_and_lse_close(out, sm_lse, out_ref, lse_ref, out_pt, lse_pt)


@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_flash_attn_varlen_tiny_dim64_gqa_causal_matches_ref(dtype):
    torch.random.manual_seed(0)
    device = "cuda"
    nheads = 8
    nheads_k = 2
    ...
    cu_seqlens_k = make_cu_seqlens(seqlens_k, device)
    q = torch.randn(sum(seqlens_q), nheads, d, device=device, dtype=dtype)
    k = torch.randn(sum(seqlens_k), nheads_k, d, device=device, dtype=dtype)
    v = torch.randn(sum(seqlens_k), nheads_k, d, device=device, dtype=dtype)
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("mode", ["max_seq5", "non_dense"])
def test_flash_attn_varlen_tiny_dim64_fallback_matches_ref(dtype, mode):
    torch.random.manual_seed(0)
    device = "cuda"
    nheads = 8
    d = 64
    ...
    if mode == "max_seq5":
        ...
        seqlens_q = [4, 3, 2]
        seqlens_k = [4, 2, 3]
        q_storage = torch.randn(sum(seqlens_q), nheads, d + 16, device=device, dtype=dtype)
        q = q_storage[..., :d]
